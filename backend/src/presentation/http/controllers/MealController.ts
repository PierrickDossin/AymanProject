...
export class MealController {
  ...
  // GET /meals?userId=xxx&date=yyyy-mm-dd - Get meals for user (optional date filter)
  list = async (req: Request, res: Response) => {
    try {
      ...
      const meals = await this.service.getMealsByUser(
        userId as string,
  ...
  // GET /meals/totals?userId=xxx&date=yyyy-mm-dd - Get daily totals
  getDailyTotals = async (req: Request, res: Response) => {
    try {
      ...
      const totals = await this.service.getDailyTotals(
        userId as string,
        date as string
      );
      res.json(totals);
